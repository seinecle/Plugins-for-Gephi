...
public class Region {
    ...
    private void updateMassAndGeometry() {
        if (nodes.size() > 1) {
            ...
            for (Node n : nodes) {
                NodeData nData = n.getNodeData();
                double distance = Math.sqrt((nData.x() - massCenterX) * (nData.x() - massCenterX) + (nData.y() - massCenterY) * (nData.y() - massCenterY)+ (nData.z() - massCenterZ) * (nData.z() - massCenterZ));
    ...
    public void setMassCenterZ(double massCenterY) {
        this.massCenterY = massCenterY;
    }
}
